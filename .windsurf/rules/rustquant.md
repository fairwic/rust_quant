...
├── infrastructure/  # 基础设施实现 (Repository、缓存)
├── services/        # 应用服务层 (业务协调)
├── market/          # 市场数据 (WebSocket、数据存储)
├── indicators/      # 技术指标计算 (纯函数)
├── strategies/      # 策略引擎 (策略实现、回测)
...
**放置规则**:
- ✅ 策略服务 → `services/src/strategy/xxx_service.rs`
- ✅ 交易服务 → `services/src/trading/xxx_service.rs`
- ✅ 市场服务 → `services/src/market/xxx_service.rs`
- ✅ 风控服务 → `services/src/risk/xxx_service.rs`
...
**禁止**:
- ❌ 直接数据库操作 (使用infrastructure)
- ❌ 直接Redis操作 (使用infrastructure)
...
### `market/` - 市场数据
**职责**: WebSocket数据流、市场数据持久化

**放置规则**:
- ✅ 数据模型 → `market/src/models/xxx_entity.rs` (FromRow) 或 `xxx_dto.rs`
- ✅ 数据访问 → `market/src/repositories/xxx_service.rs`
- ✅ WebSocket流 → `market/src/streams/websocket_service.rs`
- ✅ 交易所适配 → `market/src/exchanges/xxx.rs`
...
### 依赖方向 (单向，从上到下)
```
cli → orchestration → services → domain (接口) + infrastructure (实现)
                              ↓
...
- ❌ infrastructure 不能依赖 strategies/risk/execution/services
- ❌ indicators 不能依赖 strategies/risk/execution/services
- ❌ strategies 不能依赖 risk/execution/orchestration/services
- ❌ risk 不能依赖 strategies/execution/orchestration/services
- ❌ execution 不能依赖 strategies/orchestration/services
- ❌ services 不能直接依赖infrastructure的具体类型（应依赖domain接口）
