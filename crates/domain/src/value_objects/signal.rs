...
impl TradingSignal {
    /// 创建新的交易信号
    pub fn new(direction: SignalDirection, strength: f64, source: String, timestamp: i64) -> Self {
        Self {
            direction,
...
impl SignalResult {
    ...
    pub fn from_signals(signals: Vec<TradingSignal>) -> Self {
        ...
        // 计算平均强度
        let avg_strength =
            signals.iter().map(|s| s.strength.value()).sum::<f64>() / signals.len() as f64;
        ...
        let close_count = signals
            .iter()
            .filter(|s| s.direction == SignalDirection::Close)
            .count();
