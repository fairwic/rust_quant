...
impl IndicatorCache {
    ...
    pub async fn save<T: Serialize>(
        ...
        // 序列化为 JSON
        let json_str = serde_json::to_string(value).map_err(|e| anyhow!("序列化失败: {}", e))?;
    ...
    pub async fn get<T: DeserializeOwned>(&mut self, key: &str) -> Result<Option<T>> {
        // 从 Redis 获取
        let json_str: Option<String> = self.redis_client.get(key).await.map_err(|e| {
        ...
        match json_str {
            Some(s) => {
                // 反序列化
                let value =
                    serde_json::from_str::<T>(&s).map_err(|e| anyhow!("反序列化失败: {}", e))?;
    ...
    pub async fn delete(&mut self, key: &str) -> Result<()> {
        let deleted: i32 = self.redis_client.del(key).await.map_err(|e| {
