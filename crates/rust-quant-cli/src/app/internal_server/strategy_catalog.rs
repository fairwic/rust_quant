...
pub fn standard_strategy_catalog_items() -> Vec<StandardStrategyCatalogItem> {
    vec![
        ...
        StandardStrategyCatalogItem {
            strategy_key: "vegas_universal_4h",
            product_slug: "vegas-universal-4h",
            display_name: "Vegas 全市场自适应 4H",
            category: "趋势",
            risk_level: "中高",
            description: "基于 Vegas 4H 趋势结构，使用 ATR、成交量分位数与币种波动分层适配全市场永续合约。",
            detail: "Vegas 全市场自适应 4H 复用经过验证的 Vegas 信号引擎，并以独立策略身份管理参数、回测、信号与订阅；仅覆盖当前可交易且历史数据完整的永续合约。",
            cover_image: "/strategy-covers/strategy-trend-breakout.svg",
            display_total_return_pct: Some(101.73),
            display_sharpe_ratio: Some(1.61),
            display_trade_count: Some(139),
            display_max_drawdown_pct: Some(12.95),
            supported_exchanges: &["okx"],
        ...
        StandardStrategyCatalogItem {
            ...
            detail: "动量破位做空是独立于 market_velocity 多头主策略的新策略，只复用动量异动作为信号来源；已接入 live handoff，只有新鲜信号、用户 api-trade 订阅、交易所支持、API Key 验证、risk_ack 和保护单门禁全部通过后，才会生成实盘执行任务。",
            cover_image: "/strategy-covers/strategy-quant-core.svg",
            display_total_return_pct: None,
            display_sharpe_ratio: None,
            display_trade_count: None,
            display_max_drawdown_pct: None,
            supported_exchanges: &["okx", "binance", "hyperliquid"],
            supported_symbols: &["ALL"],
            timeframes: &["15m"],
        },
    ]
