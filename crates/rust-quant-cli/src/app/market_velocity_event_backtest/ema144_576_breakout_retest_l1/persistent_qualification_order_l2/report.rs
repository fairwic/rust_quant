//! EMA144/576 永久资格回踩 V6/V9/V12/V13 的 L2 机器报告合同。
...
pub struct EmaRetestL2Identity {
    /// 当前只属于本地多币种诊断。
...
/// L1 候选到完整交易的解析、冲突和分散性统计。
#[derive(Debug, Clone, Serialize)]
pub struct EmaRetestL2Coverage {
    /// L1 冻结候选数。
...
/// 一组按初始风险单位归一化的交易级指标。
#[derive(Debug, Clone, Serialize)]
pub struct EmaRetestL2Performance {
    /// 交易数量。
    pub trades: usize,
    /// 正 R 合计。
    pub positive_r: f64,
...
#[derive(Debug, Clone, Serialize)]
pub struct EmaRetestL2Concentration {
    /// 移除净 R 最高两笔后的剩余净 R。
...
/// 一笔 EMA144/576 回踩因果限价交易的完整 L2 证据。
#[derive(Debug, Clone, Serialize)]
pub struct EmaRetestL2TradeRecord {
    ...
    /// 当前冻结研究身份对应的目标价；V13 为信号前已确认的绝对结构价。
...
/// 预注册 L2 门禁的机器结论。
#[derive(Debug, Clone, Serialize)]
pub struct EmaRetestL2Decision {
...
/// EMA144/576 回踩候选的完整 L2 多币种本地诊断报告。
#[derive(Debug, Clone, Serialize)]
pub struct EmaRetestL2Report {
    /// 报告 schema 版本。
    pub schema_version: &'static str,
    /// 生成时间不参与策略身份。
    pub generated_at_utc: String,
    /// 冻结研究身份。
    pub identity: EmaRetestL2Identity,
    /// 冻结 L1 文件 SHA-256。
    pub source_l1_report_sha256: String,
    /// 重载行情指纹。
    pub dataset_fingerprint_sha256: String,
    /// 解析、冲突和样本覆盖。
    pub coverage: EmaRetestL2Coverage,
    /// 未扣成本绩效。
    pub gross: EmaRetestL2Performance,
    /// 8 bps/side 成本后绩效。
    pub net: EmaRetestL2Performance,
    /// 成本后多空分项。
    pub net_by_direction: BTreeMap<&'static str, EmaRetestL2Performance>,
    /// 成本后集中度。
    pub concentration: EmaRetestL2Concentration,
    /// 成交、风险、退出和成本公式是否逐笔一致。
    pub contract_identity_verified: bool,
    /// 冻结门禁结论。
    pub decision: EmaRetestL2Decision,
    /// 全量实际执行交易账本。
    pub trades: Vec<EmaRetestL2TradeRecord>,
}
