...
#[test]
fn select_stop_loss_for_v3_and_v4_keeps_atr_stop_without_fixed_three_percent_fallback() {
    let mut signal = confirmed_event(89, "opposite_96_net_decline_volume_long");
    signal.entry_price = 100.0;
    signal.structure_stop_loss_price = Some(95.0);
    signal.structure_stop_loss_source = Some("rsi_volume_regime_atr14_1_5".to_string());

    for version in [
        MARKET_RSI_VOLUME_REGIME_V3_ENTRY_RULE_VERSION,
        MARKET_RSI_VOLUME_REGIME_V4_ENTRY_RULE_VERSION,
    ] {
        let mut args = market_rsi_volume_regime_v3_research_args()
            .expect("build RSI volume regime ATR research args");
        args.paper_outcome_entry_rule_version = version.to_string();
        let selected = select_stop_loss_for_confirmed_signal(&signal, &args);

        assert_eq!(selected.price, 95.0);
        assert_eq!(selected.stop_loss_pct, 0.05);
        assert_eq!(selected.source, "rsi_volume_regime_atr14_1_5");
    }
}
