...
use rust_quant_cli::app::market_velocity_event_backtest::momentum_exhaustion_bollinger_wick_l1::recent_fast_ema_lead::run_recent_fast_ema_lead_96_l1_scan;
use std::path::PathBuf;

/// 只允许调用方指定输出路径，96 根阈值和研究身份均由模块冻结。
...
/// 执行 96 根 Research-only L1 扫描并打印最小停止边界。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let output = output_path_from_args()?;
    let report = run_recent_fast_ema_lead_96_l1_scan(&output).await?;
