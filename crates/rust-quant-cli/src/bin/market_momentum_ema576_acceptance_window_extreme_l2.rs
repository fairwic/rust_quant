...
/// 启动冻结 V24 L2；该入口只写本地研究报告，不注册任何运行态路径。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let (l1_source, v14_source, v16_source, output) = paths_from_args()?;
    run_v24_l2(&l1_source, &v14_source, &v16_source, &output).await?;
