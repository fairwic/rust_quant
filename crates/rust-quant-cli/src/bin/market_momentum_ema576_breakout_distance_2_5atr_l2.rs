...
/// 执行 V18 2.5 ATR 突破距离的 Research-only L1，并按门禁决定是否进入 L2。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let (v14_source, v16_source, output) = paths_from_args()?;
    run_v18_l1_l2_replay(&v14_source, &v16_source, &output).await?;
