...
/// 执行 V22 组合质量门禁的 Research-only L1/L2 流程。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let (v14_source, v16_source, output) = paths_from_args()?;
    run_v22_l1_l2_replay(&v14_source, &v16_source, &output).await?;
