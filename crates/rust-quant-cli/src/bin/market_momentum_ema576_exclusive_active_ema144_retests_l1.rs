...
use rust_quant_cli::app::market_velocity_event_backtest::ema144_576_breakout_retest_l1::first_breakout_pullback_hold_v1::exclusive_active_retests_v2::run_v2_l1_scan;
use std::path::PathBuf;

/// V2 只接受输出路径，禁止用命令行参数覆盖预注册生命周期或阈值。
...
/// 执行 V2 Research-only L1 扫描，不回放成交或写入数据库。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let output = output_path_from_args()?;
    let report = run_v2_l1_scan(&output).await?;
