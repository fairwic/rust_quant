...
/// 执行 V12 Research-only 首笔真实成交诊断，不写数据库或注册运行态策略。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let (l1_source, output) = paths_from_args()?;
    let report = run_v12_l2_replay(&l1_source, &output).await?;
