...
use rust_quant_cli::app::market_velocity_event_backtest::ema144_576_breakout_retest_l1::first_breakout_pullback_hold_v1::exclusive_active_retests_v2::buffered_hold_v3::finite_episode_v5::run_v5_l1_scan;
use std::path::PathBuf;

/// V5 只接受报告路径，禁止通过命令行改写预注册生命周期或形态阈值。
...
/// 执行 V5 Research-only L1 扫描，不读取成交后结果或写入数据库。
#[tokio::main]
async fn main() -> Result<()> {
    ...
    eprintln!(
        "L1 status={} candidates={} effective_events={} target_gates={}/{} btc_episode_interrupt={}",
