...
/// 执行 V11 Research-only L2 成本诊断，不写数据库或注册运行态策略。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let (l1_source, output) = paths_from_args()?;
    let report = run_v11_l2_replay(&l1_source, &output).await?;
