...
/// 执行 V10 Research-only L1 扫描，不读取成交后结果或写入数据库。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let output = output_path_from_args()?;
    let report = run_v10_l1_scan(&output).await?;
