...
/// 启动研究回放；该入口不注册 Pine、worker 或任何生产路径。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let (v14_source, v16_source, output) = paths_from_args()?;
    run_v23_l1_l2_replay(&v14_source, &v16_source, &output).await?;
