...
/// 启动 V26 无 outcome L1，不注册任何运行态路径。
#[tokio::main]
async fn main() -> Result<()> {
    dotenv::dotenv().ok();
    let (v25_source, v14_source, output) = paths_from_args()?;
    run_v26_l1(&v25_source, &v14_source, &output).await?;
