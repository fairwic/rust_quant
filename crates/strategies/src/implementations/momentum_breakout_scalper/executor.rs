use super::strategy::MomentumBreakoutScalperStrategy;
use super::types::{MomentumBreakoutSignalSnapshot, MomentumBreakoutThresholds};
...
/// Momentum Breakout Scalper 的 live/paper 执行器；只识别带版本的 v1 key。
pub struct MomentumBreakoutScalperStrategyExecutor;

impl MomentumBreakoutScalperStrategyExecutor {
    pub fn new() -> Self {
        Self
    }
}

impl Default for MomentumBreakoutScalperStrategyExecutor {
    fn default() -> Self {
        Self::new()
    }
}
...
#[serde(default)]
struct MomentumBreakoutExecConfig {
    thresholds: MomentumBreakoutThresholds,
    snapshot: Option<MomentumBreakoutSignalSnapshot>,
}

#[async_trait]
impl StrategyExecutor for MomentumBreakoutScalperStrategyExecutor {
    ...
    fn can_handle(&self, strategy_config: &str) -> bool {
        ...
        strategy_key(&value).is_some_and(|key| matches!(key, "momentum_breakout_scalper_v1"))
    }
    ...
    async fn execute(
        ...
        let config: MomentumBreakoutExecConfig =
        ...
        let Some(mut snapshot) = config.snapshot.clone() else {
            return Ok(MomentumBreakoutScalperStrategy::flat_missing_snapshot(
                price, ts,
            ));
        };
        ...
        let decision = MomentumBreakoutScalperStrategy::evaluate(&config.thresholds, &snapshot);
