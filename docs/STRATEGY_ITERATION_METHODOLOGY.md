...
> 日常新想法默认先按 [STRATEGY_RESEARCH_WORKFLOW.md](STRATEGY_RESEARCH_WORKFLOW.md)
> 执行 L0/L1；只有结果正向并升级到 L3，才进入本文的完整闸门，避免每个截图或阈值直接触发全量验证。
...
4. 最小回测 (1 币种, 2 个月)
5. 决策点:
   - 净 EV > 0、净 PF > 1，且无时序泄漏 → 进入生产模式继续验证
   - 净 EV 接近 0 或成本后转负 → 仅保留研究候选，先做归因
   - 净 EV <= 0 或净 PF <= 1 → 记录失败，下一个想法
```
...
# 看联合指标；探索样本只决定是否继续研究，不代表可上线
Win Rate: 58.3%
Net EV: 0.42R         # 正期望，但未达职业级 0.6R
Net PF: 1.85          # 有研究价值，但未达职业级 2.2
Total Trades: 23      # 仅为探索样本
Max DD: 6.7%
```
...
```text
if 净 EV > 0 且净 PF > 1 且没有时序泄漏:
    标记为“研究候选”，进入完整样本外回测
else:
    记录证伪原因，停止晋级

只有 Gate 4 的职业级联合指标与稳健性检查全部通过，才进入 Paper/ReadOnly。
```
...
## 探索模式结果
- Win Rate: 58.3%（描述指标，不单独判定）
- Net EV: 0.42R（研究候选，未达职业级门槛）
- Net PF: 1.85（研究候选，未达职业级门槛）
- 币种: BTC 4H
...
**何时升级**：
- 探索模式的净 EV、净 PF 显示正向研究价值，且没有时序泄漏
- 用户需求明确（不是纯研究）
- 准备投入 1-2 周资源
...
- 1-2 个月数据快速验证
- 净 EV > 0、净 PF > 1 且无时序泄漏 → 进入生产模式继续完整验证；此时仍不得宣称达到职业级晋级标准
...
策略不得再用“胜率 > 55%”或单次 PnL 作为晋级依据。以下指标必须在严格时序、计入手续费/滑点/资金费的样本外或 walk-forward 结果上联合判定：

| 指标 | 晋级目标 |
| --- | --- |
| 胜率 | 参考 `40%~65%`，常规目标 `45%~65%`；不单独判定 |
| 实际盈亏比 | 参考 `1.8~3.0`；必须与 EV、PF 联合解释 |
| 每笔期望（EV） | 净值 `>= 0.6R` |
| Profit Factor | 净值 `>= 2.2` |
| 最大回撤 | `<= 10%`，按逐时盯市权益计算 |
| Recovery Factor | `>= 4` |
| Sharpe Ratio | `>= 1.5`，优秀 `>= 2`；默认日收益、`sqrt(365)` 年化 |
| `1m~15m` 频率 | `50~120` 笔/月，`2~5` 笔/天仅作观察 |
| `4H~1D` 频率 | `8~40` 笔/月，`0.3~2` 笔/天仅作观察 |

在平均亏损为 `1R`、未计成本时，联合参考组合为：

| 胜率 | 盈亏比 | 毛 EV | 毛 PF | 结论 |
| --- | --- | --- | --- | --- |
| 40% | 3.0 | `0.60R` | `2.00` | PF 未达晋级线 |
| 45% | 2.5 | `0.575R` | `2.05` | 近门槛研究组合，不晋级 |
| 50% | 2.2 | `0.60R` | `2.20` | 理论触线，需扣除成本复核 |
| 55% | 2.0 | `0.65R` | `2.44` | 理论达标，需稳健性验证 |
| 60% | 1.8 | `0.68R` | `2.70` | 理论达标，重点审查样本与泄漏 |

交易频率以完整月份为主，并分别报告 `strategy_version x symbol x timeframe` 与组合汇总口径。频率用于检查样本可信度、成本和容量。频率不足时优先验证预先定义的可交易币种池能否提供足够机会；放宽信号必须作为独立版本和新假设重新走样本外验证，不得为了凑交易数直接修改当前版本。

`1H` 等不属于 `1m~15m` 或 `4H~1D` 的周期必须在参数搜索前单独声明频率目标，不得看完回测结果后再套用更宽松的频率区间。

#### 4.5.1 全市场扫描策略的组合口径

全市场扫描策略以 `strategy_version x universe_version x timeframe` 的真实可执行组合频率为主要频率口径；单币种频率只用于分布诊断，不要求每个币种分别达到月频目标。单币种/专用策略仍以 `strategy_version x symbol x timeframe` 为主要口径。

例如，预先定义的 30 个币种平均每月各产生 2 笔真实可执行交易时，组合可记录约 60 笔/月。但该结论必须同时满足以下规则：

1. **币种池版本化与历史重建**：研究前固定交易所、产品类型、报价币、流动性/成交额、点差、上市时长、数据完整度、交易能力、纳入/剔除及生效时间。历史回放只使用当时可见成员并保留退市币，禁止当前成员列表回放和事后挑选盈利币种。
2. **真实可执行交易**：频率只统计经过币种池、流动性、交易所 symbol filters、策略风控、容量、精度和并发限制后实际能够开仓的交易。原始命中、阻塞信号和因容量被淘汰的候选分别统计。用户订阅、凭证或账户 readiness 造成的阻塞另记为运行态可执行频率，不得反向改变策略研究频率。
3. **生产同口径排序**：同一时点候选超过容量时，只能按信号时点可见、确定、可复现的排序选择 Top N；回测不得假设无限资金或事后选择最终涨幅最大的候选。
4. **有效市场事件与样本闸门**：同时报告原始成交笔数与按触发时间、方向、市场状态、板块/相关性聚类后的有效事件数，且在查看结果前把聚类时间窗、相关性阈值和归并规则写入 evaluation manifest。同一轮动量中的多币种交易可以算多笔执行，但不能当成同等数量的独立统计样本。Gate 4 的交易数量可以按组合实际成交数统计，不要求每个币种达到固定最小笔数，但必须同时满足有效事件数量、币种/Tier 覆盖和市场状态覆盖；缺少某一层覆盖时不得宣称对该层全局适用。
5. **真实组合权益**：统一资金曲线必须落实最大同时持仓、单币风险、同方向/同板块/高相关簇风险、总敞口、杠杆、流动性、费用和滑点约束。最大回撤、Recovery 与 Sharpe 以该组合权益计算，组合频率以相同资金和容量约束下的实际执行交易计算。
6. **双层指标**：净 EV 与净 PF 同时报告交易级聚合和组合级结果；置信区间、显著性和稳健性判断使用聚类后的有效样本，避免相关交易虚增样本量。
7. **集中度审计**：按币种、Tier、月份、市场状态和相关簇报告贡献，并执行移除头部盈利交易/币种、限制单币贡献和季度窗口检查。若结果主要由少数币种或单次事件贡献，降级为研究候选或重新声明为专用策略。

每笔交易的 `1R` 固定为入场时初始止损对应的风险金额，即 `abs(入场价 - 初始止损价) x 实际数量`。净 R 倍数按扣除全部交易成本后的已实现 PnL 计算，移动止损不得回写初始 `R`。报告必须记录初始资金、单笔风险比例、最大并发风险、杠杆、资金分配和复利/固定仓位方式；未固定同一仓位模型时，不得横向比较回撤、Recovery 或组合收益。无有效初始止损的交易不得进入职业级指标样本。

晋级还必须通过参数邻域、不同 K 线时间窗口、收益集中度、移除头部盈利交易/币种、费用与滑点压力和跨币种验证。专用币种策略可以保留，但必须声明边界和独立版本；不得将单币种特调结果包装为通用策略。

### 4.6 回测结果的可信度评估（更新：跨 Tier 对比）
...
- 积累信号数 ≥ 30 笔
- 审核通过率 ≥ 80%
- 已执行信号扣除成本后保持净 EV > 0、净 PF > 1，且未相对样本外基线出现不可解释的显著恶化
- 已执行信号的 Max DD < 10%
- Gate 4 的职业级联合指标和稳健性闸门已经通过；Paper 的 30 笔样本只作运行态校验，不替代完整回测样本
