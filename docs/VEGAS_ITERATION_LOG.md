...
- **规则目的**：避免“局部样本修对了，但整体基线和跨币种表现退化”的改动被过早升级为正式基线。
- **适用范围**：所有从 `ETH` 单币种样本出发的新规则、窄过滤、止损链调整、形态识别修正。

### 执行顺序

0. **默认自主推进执行层动作**
   - 代码修改后，默认直接执行：编译、回测、结果查询、对比复盘、日志更新。
   - 只有遇到“判断分支走不通”或“需要切换大优化方向”时，才停下来向用户确认。

1. **先在 ETH 上完成单币种闭环**
   - 必须先回答：这条改动在 `ETH` 当前基线里是否是**净正向**。
   - 判断口径至少包含：`profit`、`sharpe_ratio`、`max_drawdown`、`win_rate`。
   - 只修单样本、但整体 ETH 回测退化的改动，不能晋级为正式基线。

2. **ETH 确认正向后，再做跨币种复核**
   - 至少复核 `BTC / SOL / BCH`。
   - 目的不是要求每个币都大幅提升，而是确认这条改动不会把其他币种明显拖坏。
   - 若跨币种结果 mixed，则保持“ETH 候选”状态，不直接升级为正式跨币种基线。

3. **只有在 ETH 正向 + 跨币种不拖坏后，才允许更新基线**
   - 才允许更新数据库默认配置。
   - 才允许在迭代日志中写入“新基线 / 推荐基线 / 已确认基线”。
   - 才允许把该改动并入后续参数优化的默认起点。

### 明确禁止

- **禁止**：ETH 仍为负向或 mixed 时，就提前把规则扩展到其他币种继续优化。
- **禁止**：只凭单个案例修复，就写入“基线升级”或“推荐默认”。
- **禁止**：其他币种尚未复核，就把单币种实验结果记成正式跨币种结论。
- **禁止**：执行层动作反复等待确认，例如“是否编译 / 是否跑回测 / 是否查询结果 / 是否写日志”。

## 2026-04-15 波动性分层普适性规则（新增）

- **规则目的**：避免把“只对 ETH 有效”的量化因子误写成跨币种通用结论，同时保留同一结构逻辑在不同波动层做参数分层验证的空间。
- **分层前提**：默认按波动性将验证对象分成三类：
  - `BTC`
  - `ETH`
  - `其他币种`
- **约束依据**：当前仓库的实验经验已多次表明，`BTC 波动性 < ETH 波动性 < 其他币种`，因此不能要求同一套阈值、权重、止损参数在三层都同时最优。

### 执行规则

1. **结构逻辑先在 ETH 确认**
   - 新因子、过滤器、止损修正仍然先走 ETH 闸门。
   - 若 ETH 本身不成立，不进入“普适性”讨论。

2. **跨币种复核默认采用三层口径**
   - ETH 通过后，再分别复核 `BTC`、`ETH`、`其他币种`。
   - 复核时允许做**分层参数调整**，例如不同层使用不同的：
     - `volume_ratio` 阈值
     - `macd histogram` 阈值
     - 止损收紧幅度
     - 形态质量下限
...
3. **结论必须分级，不得混写**
   - `单参数通用`：同一参数组跨三层都成立。
   - `分层参数通用`：同一结构逻辑成立，但需按 `BTC / ETH / 其他币种` 分层调参。
   - `仅 ETH 有效`：ETH 成立，但其他层在合理分层调参后仍不成立。

4. **晋级约束**
   - 只有完成三层复核后，才允许把规则写成“分层参数通用”或“单参数通用”。
   - 若只有 ETH 有效，不得借口“其他币种波动不同”直接升级为跨币种基线。

### 记录要求

- 后续日志必须明确写出：本轮结论属于 `单参数通用`、`分层参数通用` 还是 `仅 ETH 有效`。
- 若做了分层调参，必须记录各层参数差异与对应回测结果，不能只写一个总判断。

### 本轮复盘触发该规则的原因

- 吞没形态链路这轮属于典型案例：
  - 局部样本（`2026-03-25 20:00:00` 的 ETH short）确实被修正；
  - 但 ETH 整体回测从 `1279` 到 `1283 / 1290 / 1294 / 1298` 仍未恢复到原基线水平；
  - 同时 `BTC / SOL / BCH` 的复核也没有给出明确净正向结论。
- 因此这类改动当前只能继续作为 **ETH 候选实验**，不能升级为正式基线，也不能写入“跨币种已确认”。
...
- 先前吞没链路的大改动没有带来净正向优化，已全部回滚。
- 本轮只保留一条最小规则：
  - 若出现吞没形态，但 **收盘实体 / 整根K线振幅 < 0.5**，则直接视为非吞没。
- 执行方式按新规则直接完成：代码修改 -> 编译 -> 回测 -> 结果复盘，无额外等待确认。

#### 代码范围

- [engulfing.rs](/Users/mac2/onions/rust_quant/crates/indicators/src/pattern/engulfing.rs)
...
| 1302 | 仅保留 `body/range < 0.5` 非吞没 | 52.39% | 4569.84 | 2.4988 | 36.53% |

#### 观察

- 你此前关注的东八区 `2026-03-25 20:00:00` ETH short，在 `1302` 中已经不存在，说明该小实体长影线样本被这条规则直接过滤掉。
- 但按 ETH 先行闸门，这次实验仍然 **不是净正向**：
  - `profit -2557.64`
  - `sharpe -0.4477`
  - `max_drawdown +0.77` 个百分点
  - `win_rate` 基本持平
...
| ----------- | ---- | ---- | ---- | ------ | -------- |
| 1303 | BTC | 54.56% | 207.44 | 0.7700 | 49.80% |
| 1304 | SOL | 43.94% | 495.10 | 1.0688 | 40.78% |
| 1305 | BCH | 35.54% | -82.83 | -0.6568 | 87.72% |

#### 结论

- 这条规则修正了目标样本，但 ETH 整体收益仍明显低于 `1279`。
- 因此本轮结论是：
  - **不升级为 ETH 正式基线**
  - **不进入跨币种晋级流程**
- **保留为已验证但拒绝晋级的候选规则**
...
- 为 Vegas 后续引入资金费率、持仓量、premium、链上净流入等外部特征做基础设施铺设
- 优先顺序：
  - Hyperliquid `fundingHistory + metaAndAssetCtxs`
  - Dune `Ethereum/Hyperliquid` 查询模板
  - OKX/Binance 扩展接口占位

#### 本轮落地内容

- 领域层新增 `ExternalMarketSnapshot` 与 `ExternalMarketSnapshotRepository`
- 新增表：
  - `external_market_snapshots`
  - 唯一键：`(source, symbol, metric_type, metric_time)`
- Infrastructure 新增：
...
#### Hyperliquid SDK 方案备注

- 初版先用手写 REST 适配验证了接口和落库链路
- 随后按要求切换为 **官方 Rust SDK**
- 当前使用的是官方仓库 `hyperliquid-dex/hyperliquid-rust-sdk`
- 之所以不用 crates.io `0.6.0` 发布版，是因为该版本缺少 `metaAndAssetCtxs`
- 现在 `HyperliquidPublicAdapter` 已经改为基于官方 SDK `InfoClient`

#### 当前可用能力

- 可解析 Hyperliquid 历史 funding 行
- 可解析 Hyperliquid 当前 funding/premium/open interest/mark/oracle 快照
- 可将 funding/meta 统一映射为 `ExternalMarketSnapshot`
- 可按唯一键幂等写库
- 已为 Dune 提供模板，并补上最小 API 执行链
- 已为 `okx/binance` 预留 source 和 symbol 归一化扩展点，但尚未实现 provider

#### 下一步

- 将 Dune 模板执行挂到 orchestration job 或 CLI 入口
- 在服务层补 `OKX/Binance` provider 分支
- 等外部数据稳定沉淀后，再回到 Vegas 信号层引入特征，不直接在本轮接入交易信号
...
- 新增 `DuneMarketSyncService`
  - 模板参数替换
  - 结果轮询
  - 行到 `ExternalMarketSnapshot` 的映射
- 验证新增：
...
#### 2026-04-09 Dune 真实执行补充

- 用户提供 `DUNE_API_KEY` 后，已打通真实 Dune 执行与入库闭环
- 先后定位并修正了 4 个真实问题：
  - `hyperliquid_funding_basis.sql` 引用了不存在的 `dune_user_generated.hyperliquid_perp_hourly`
  - ad-hoc SQL 的 `results` 响应可能没有 `query_id`，解析器原先会报 `missing query_id`
  - Dune 时间过滤需要 `from_iso8601_timestamp(...)`，不能直接 `CAST('...Z' AS TIMESTAMP)`
  - Dune 返回 `2026-02-21 20:00:00.000 UTC` 这类时间格式，服务层原先无法解析
- 真实验证命令：
  - `cargo test -p rust-quant-infrastructure --test dune_client -- --nocapture`
  - `cargo test -p rust-quant-services --test dune_market_sync -- --nocapture`
...
- 真实入库结果：
  ...
  - `source=dune`
  - `symbol=ETH`
  - `metric_type=hyperliquid_basis`
  - 成功写入 4 条小时聚合记录，时间为 `2026-02-21 20:00:00 UTC` 到 `2026-02-21 23:00:00 UTC`
- 数据边界确认：
  - `hyperliquid.market_data where coin='ETH'` 当前最早 `2023-05-20 02:50:04 UTC`
  - 当前最晚 `2026-02-21 23:59:00 UTC`
  - 因此 `2026-03-30` 窗口查不到数据，不是同步链路失败，而是 Dune 数据尚未覆盖到该时间点

#### 2026-04-09 Dune 模板调度入口补充

- 已将 Dune 模板同步挂入主程序 `run_modes()` 的数据同步分支
- 触发条件：
  - `IS_RUN_SYNC_DATA_JOB=1`
  - `IS_RUN_DUNE_SYNC_JOB=1`
- 现已支持：
  - `SYNC_SKIP_MARKET_DATA=1`，只跑外部数据同步，不跑 K 线同步
- 支持两种配置方式：
  - 单任务环境变量：`DUNE_METRIC_TYPE / DUNE_SYMBOL / DUNE_TEMPLATE_PATH / DUNE_START_TIME / DUNE_END_TIME`
  - 批量任务环境变量：`DUNE_TEMPLATE_JOBS`
- `DUNE_TEMPLATE_JOBS` 格式：
  - `metric_type|symbol|template_path|start_time|end_time|performance|[min_usd]`
  - 多任务用 `;` 分隔
- 兼容性补充：
  - orchestration job 失败时现在会真正返回错误，不再只打日志后继续
- 验证：
  ...
  - `cargo run --bin rust_quant` 配合 `IS_RUN_SYNC_DATA_JOB=1 IS_RUN_DUNE_SYNC_JOB=1`
  - 实测 `SYNC_SKIP_MARKET_DATA=1 SYNC_SKIP_TICKERS=1 EXIT_AFTER_SYNC=1` 时：
    - 跳过 K 线同步
    - 执行 Dune 模板同步
    - `saved=4`
    - 主程序优雅退出
...
- 背景：
  - 分析 `ETH 1343` 东八区 `2026-03-27 00:00:00` 开空样本
  - 该样本同时满足：
    - `Bolling long`
    - `Engulfing short`
    - `LegDetection bearish + is_new_leg=true`
    - `ema_distance.state = TooFar`
    - `fib.in_zone = true`
    - `volume_ratio = 1.0627`
  - 聚类后发现：
    - 所有 `Bolling long + Engulfing short + bearish leg` 的 short 在 ETH 整体为正
    - 但再收窄到 `is_new_leg=true + TooFar + fib.in_zone=true + 低量` 后，目标坏簇明显转负
- 实验内容：
  - 新增窄过滤 `CONFLICTING_TOO_FAR_NEW_BEAR_LEG_SHORT`
  - 仅拦截满足以下全部条件的 short：
    - `Bolling long`
    - `Engulfing short`
    ...
    - `volume_ratio < 1.5`
  - 采用 TDD：
    - 先补 `conflicting_too_far_new_bear_leg_short_should_be_blocked_with_low_volume`
    - 再补 `conflicting_too_far_new_bear_leg_short_should_stay_allowed_with_high_volume`
- 验证：
  - `cargo test -p rust-quant-indicators conflicting_too_far_new_bear_leg_short_should -- --nocapture`
  - `cargo test -p rust-quant-indicators trend::vegas::strategy::tests -- --nocapture`
  - `cargo build --bin rust_quant`
  - `IS_BACK_TEST=1 IS_RUN_SYNC_DATA_JOB=0 TIGHTEN_VEGAS_RISK=0 DB_HOST='mysql://root:example@localhost:33306/test?ssl-mode=DISABLED' ./target/debug/rust_quant`
- 样本确认：
  - `1343` 中该时间点有开空：
    - `2026-03-27 00:00:00 -> 2026-03-27 04:00:00`
    - `Signal_Kline_Stop_Loss / Engulfing_Volume_Rejected`
    - `profit_loss = -81.8008`
  - `1351` 中该开空已不存在
  - `filtered_signal_log 1351` 中新增：
...
- 回测结果：
  ...
  - `1345 -> 1353 (SOL)`: `43.6860% / 500.31 / 1.15575 / 41.7609%` -> `43.6860% / 500.31 / 1.15575 / 41.7609%`
  - `1346 -> 1354 (BCH)`: `36.1301% / -75.39 / -0.53237 / 79.7638%` -> `36.1921% / -75.72 / -0.53668 / 80.0387%`
- 结论：
  - ETH 单币种闸门通过
  - BTC 同向改善，SOL 不变
  - BCH 仅轻微退化，当前不构成“明显拖坏”
  - 该规则升级为当前正式基线
  - 当前正式基线回测 ID：
    - `ETH 1351`
    - `BTC 1352`
    - `SOL 1353`
    - `BCH 1354`
  - 后续继续观察 BCH；若后续出现同类退化簇，再单独收窄，不回退本轮基线

### 2026-04-14: TooFar 反趋势锤子线 long 广义放行（已验证，拒绝晋级）

- 目标：
  - 不只放 `2026-03-30 04:00:00` 单点
  - 从 `EMA_TOO_FAR_COUNTER_TREND_HAMMER_LONG` 命中的母集中，寻找更通用、且仍能保持盈利的放行条件
- 历史筛选结论：
  - `ETH 1351` 中该过滤命中 `30` 笔，shadow pnl 合计 `-1.0941`
  - 若收窄到 `非横盘 + short_trend=true + above_zero=false + volume_ratio>=2.5`，共有 `8` 笔，只有 `2026-03-30 04:00:00` 明显为正，其余 `7` 笔均为负
  - 在母集上做阈值搜索后，最宽还能保持净正向的候选为：
    - `volume_ratio >= 2.8`
    - `RSI >= 36`
- 实验内容：
  - 仅在 `EMA_TOO_FAR_COUNTER_TREND_HAMMER_LONG` 前增加一个广义豁免
  - 条件：
    - `!is_ranging_market`
    - `volume_ratio >= 2.8`
    - `RSI >= 36`
...
- 验证：
  ...
  - `IS_BACK_TEST=1 IS_RUN_SYNC_DATA_JOB=0 TIGHTEN_VEGAS_RISK=0 DB_HOST='mysql://root:example@localhost:33306/test?ssl-mode=DISABLED' ./target/debug/rust_quant`
- 目标样本变化：
  - `1351` 中东八区 `2026-03-30 04:00:00` 未开多
  - `1355` 中该笔已开多，并在 `2026-03-30 08:00:00` 以 `Signal_Kline_Stop_Loss / Engulfing_Volume_Confirmed` 盈利出场，`profit_loss=+35.8730`
- 回测结果：
  - `1351 -> 1355 (ETH)`: `52.3901% / 7090.69 / 2.92250 / 35.7654%` -> `52.3810% / 6756.63 / 2.88018 / 35.7654%`
  - `1352 -> 1356 (BTC)`: `54.4861% / 340.10 / 1.04279 / 36.8281%` -> `54.4861% / 343.58 / 1.04926 / 36.3271%`
  - `1353 -> 1357 (SOL)`: 无变化
  - `1354 -> 1358 (BCH)`: `36.1921% / -75.72 / -0.53668 / 80.0387%` -> `36.3014% / -75.50 / -0.53395 / 79.8579%`
- 结论：
  - 目标样本放对了
  - `BTC / BCH` 边际改善，`SOL` 不变
  - 但 `ETH` 利润与 Sharpe 明显下降，未通过单币种闸门
  - 实验代码已撤回，不更新正式基线
...
- 目标样本变化：
  - `1371` 与 `1375` 中，东八区 `2026-03-30 04:00:00` 都不再形成 `LONG`
  - 该样本既没有进入 `filtered_signal_log`，也没有开仓记录
  - `vegas_kline_debug` 显示它被直接压成 `direction=None`
- 回测结果：
  - `1351 -> 1371 (ETH, ratio=1.0)`: `52.3901% / 7090.69 / 2.92250 / 35.7654%` -> `52.1008% / 203.18 / 0.74011 / 43.6496%`
  - `1351 -> 1375 (ETH, ratio=0.63)`: `52.3901% / 7090.69 / 2.92250 / 35.7654%` -> `51.0791% / 721.21 / 1.23207 / 37.5598%`
  - `1376 / 1377 / 1378` 对 `BTC / SOL / BCH` 基本无改善，不足以抵消 ETH 退化
- 交易归因：
  - `1351 -> 1375` 被删掉的 `short` 共 `126` 笔，合计 `-511.3948`
  - 被删掉的 `long` 共 `140` 笔，合计 `+1938.8090`
  - 说明全局冲突惩罚不是只在砍坏单，而是大量误伤盈利 `long`
- 结论：
  - 方向判断本身成立：目标样本确实属于“靠反向权重抬分过线”
  - 但“全局冲突惩罚”作用面过大，会重写 ETH 的收益结构
  - 实验代码已撤回，数据库配置已恢复
  - 当前正式基线仍保持 `1351 / 1352 / 1353 / 1354`
...
### 2026-04-15: 历史迭代总结与快速迭代协议沉淀

- 历史日志里最稳定的失败模式：
  - 规则从单笔样本倒推，但没有先验证“是否真正命中目标样本”
  - ETH 单币结果尚未净正向，就提前扩到其他币种
  - 四币一次性全跑，导致 token 浪费、写库阻塞、归因困难
  - 先拉全量明细再想问题，导致查询重、token 高、结论反而不清
  - 边界结果（胜率/Sharpe 升、profit 微降）直接二选一，而不是先做一轮最小阈值收敛
- 历史日志里最稳定的成功模式：
  - 先从正式基线里找坏簇/好簇，再写最小规则
  - 先看命中数和 shadow pnl，再决定是否值得全量回测
  - 先跑 `ETH`，再做 `BTC / ETH / 其他币种` 的分层复核
  - 规则只改一个点，回测与归因都更清楚
  - 需要跨币时，不强行单参数通用，而是收敛成 `分层参数通用`
- 本次沉淀到规则与技能中的快速迭代协议：
  - 一轮只验证 1 个假设
  - 顺序固定为：`目标样本 -> ETH -> 分层复核`
  - 跨币默认单币顺序跑，不默认四币合跑
  - 过滤类规则先查 `filtered_signal_log` 命中与 shadow pnl
  - 每轮实验默认只保留：`假设 / 命中结果 / ETH delta / 分层结果 / 分类结论`
  - 默认只看日志尾部和相关实验段，不重复扫全量历史
  - 测试库空间不足时，允许删除已拒绝实验数据，避免阻塞当前迭代
- 落地位置：
  - `AGENTS.md`
  - `vegas-backtest-runbook/SKILL.md`
...
- 假设：
  - 当前正式基线 `1424 / 1426 / 1427 / 1423` 中，存在一类被过度放行的 short：
    ...
    - `macd.histogram > 0`
    - `RSI >= 55`
  - 这类本质上是在大级别上升趋势里、价格已 `TooFar` 的位置，靠单根对立 `hammer short` 去追空，容易形成坏 short。
- 目标样本：
...
- TDD / 验证：
  ...
  - `cargo build --bin rust_quant`
- ETH 单币验证：
  - `BACKTEST_ONLY_INST_IDS='ETH-USDT-SWAP' ...` -> `1428`
  - `1424 -> 1428`：
    - `win_rate`: `52.3992% -> 52.9183%`
    - `profit`: `8061.41 -> 8047.17`
    - `sharpe`: `3.10425 -> 3.14525`
    - `max_drawdown`: `35.0947% -> 32.3410%`
  - 命中：
    - `35` 笔
    - shadow PnL `-0.9974`
  - 判断：
    - 虽然 `profit` 轻微回落，但 `win_rate / sharpe / max_drawdown` 同向改善，且 shadow PnL 明显为负，属于可接受收敛。
- 分层复核：
  - `BTC 1426 -> 1429`：
    - `54.3230% / 336.744 / 1.03625 / 37.3102%`
    - `-> 54.3514% / 341.636 / 1.04603 / 37.3102%`
    - 命中 `29` 笔，shadow PnL `-0.8235`
  - `SOL 1427 -> 1430`：
    - 完全不变，仍为 `42.7609% / 715.006 / 1.44106 / 41.4416%`
    - 命中 `0` 笔
  - `BCH 1423 -> 1431`：
    - `34.6484% / -76.4805 / -0.556099 / 80.8969%`
    - `-> 34.4887% / -72.5423 / -0.473708 / 77.2249%`
    - 命中 `18` 笔，shadow PnL `+0.0936`
- 结论：
  - 这轮可接受并升级基线。
  - 分类：`单参数通用`
    - 同一参数规则在 `ETH / BTC / BCH` 同向改善；
    - `SOL` 不命中，但未被拖坏。
  - 新正式基线升级为：
    - `ETH 1428`
    - `BTC 1429`
...
- 假设：
  - 在正式基线 `1428 / 1429 / 1430 / 1431` 里，存在一类 long 坏簇：
    - `ema_distance.state == TooFar`
    - `fib.in_zone == true`
    - `boll_short == true && boll_long == false`
    - `leg.is_bullish_leg == true`
...
- ETH 单币：
  ...
  - 命中 `3` 笔，shadow PnL `-0.0715`
- 分层复核：
  - `BTC 1429 -> 1433`：完全不变
  - `SOL 1430 -> 1434`：
    - `42.7609% / 715.006 / 1.44106 / 41.4416%`
    - `-> 42.4138% / 587.749 / 1.30134 / 41.4416%`
    - 命中 `12` 笔，shadow PnL `-1.3235`
- 结论：
  - `ETH` 单币正向，但 `SOL` 明显退化。
  - 这说明该坏簇的 shadow PnL 为负，但真实资金曲线副作用过强。
  - 按当前规则判定为：`已验证但无效`
  - 代码已回滚，正式基线保持：
    - `ETH 1428`
    - `BTC 1429`
...
- 假设：
  - 当前基线中存在一类被过度放行的 long：
    ...
    - `!ema_touch.is_uptrend`
    - `!hammer_long`
  - 想按波动性分层规则，使用 `RSI / MACD histogram / volume_ratio` 作为代理阈值，避免直接按币种硬编码。
...
- 第一轮代理：`RSI >= 50`（旧基线 `1403 / 1404 / 1405 / 1406`）
  ...
  - 命中：
    ...
    - `BTC` `0` 笔
  - 结论：
    - 虽然 `ETH` 的 `win_rate / sharpe` 提升，`SOL / BCH` 也改善，但 `ETH profit` 仍轻微回落。
    - 按 `ETH` 先行净正向闸门，不升级。
...
- 第二轮代理：`RSI >= 55 && histogram >= 5 && volume_ratio >= 2.8`（正式基线 `1428 / 1429 / 1430 / 1431`）
  ...
  - 命中：
    ...
    - `BTC / SOL` `0` 笔
  - 结论：
    - 继续收窄后，`BTC` 确实不再受影响，但 `ETH` 利润和 Sharpe 明显退化。
    - 说明这条 long 坏簇即使做分层代理，也不具备稳定的可交易改进。
    - 判定：`已验证但无效`
    - 代码已回滚，正式基线保持：
      - `ETH 1428`
      - `BTC 1429`
...
- 假设：
  - 在正式基线 `1428 / 1429 / 1430 / 1431` 中，存在一类 short 坏簇：
    - `ema_touch.is_uptrend == true`
    - `!ema_values.is_short_trend`
...
- 结论：
  - 虽然 shadow PnL 全为负，但真实回测里 `ETH` 利润、Sharpe、回撤同时退化。
  - `SOL` 也明显变差，`BCH` 回撤更差。
  - 判定：`已验证但无效`
  - 代码已回滚，正式基线保持：
    - `ETH 1428`
...
- 假设：
  - 当前正式基线 `1428 / 1429 / 1430 / 1431` 中，一类 long 坏簇可能不该开：
    - `ema_distance.state == TooFar`
    - `!ema_touch.is_uptrend`
    - `!ema_values.is_long_trend`
    - `!ema_values.is_short_trend`
...
- 结论：
  - 虽然开仓坏簇聚合是负的，但真实回测里 `ETH` 利润和 Sharpe 回落，`SOL` 也退化。
  - `BTC` 不变，`BCH` 只有轻微改善，不足以抵消 `ETH/SOL` 退化。
  - 判定：`已验证但无效`
  - 代码已回滚，正式基线保持：
    - `ETH 1428`
    - `BTC 1429`
...
- 背景：
  - 研究系统第一版已经能同时对比 `已成交样本` 与 `过滤候选`，但结论层之前按整个因子家族汇总，容易把真正有效的单桶环境稀释掉。
- 本次实现：
  - 将研究系统的结论判定从“按因子家族”改成“按 `因子 + 桶 + 波动层`”逐桶判断。
  - 当前会把真正满足 `ETH 先行闸门` 且明显优于过滤候选的具体环境桶标记为 `可回注`。
- 验证：
  - `cargo test -p rust-quant-services --test vegas_factor_research -- --nocapture`
  - `DB_HOST='mysql://root:example@localhost:33306/test?ssl-mode=DISABLED' cargo run -p rust-quant-cli --example run_vegas_factor_research`
- 真实结果：
  - 当前正式基线 `1428 / 1429 / 1430 / 1431` 下，首个被研究系统明确筛出的候选环境是：
    - `funding_premium_divergence / funding_positive / ETH`
  - 分桶表现：
    - `已成交样本`：`10` 笔，`win_rate=80.00%`，`avg_pnl=134.25`，`SharpeProxy=0.70`
    - `过滤候选`：`20` 笔，`win_rate=35.00%`，`avg_pnl=0.01`，`SharpeProxy=0.12`
- 下钻结论：
  - 这不是“所有 funding positive 都该放开”，而是一个方向性环境：
    - 已成交样本中 `ETH funding_positive` 下：
      - `long`：`8` 笔，`pnl=749.6918`，`avg_pnl=93.7115`，`win_rate=75.00%`
      - `short`：`2` 笔，`pnl=592.7788`，`avg_pnl=296.3894`，`win_rate=100.00%`
    - 过滤候选中：
      - `LONG`：`8` 笔，`avg_pnl=0.0328`，`win_rate=50.00%`
      - `SHORT`：`12` 笔，`avg_pnl=-0.0071`，`win_rate=25.00%`
  - 过滤理由分布显示：
    - 被拦 `LONG` 里，`MACD_FALLING_KNIFE_LONG` 在 funding positive 环境下反而整体为正
    - 被拦 `SHORT` 里，`EMA_TOO_FAR_OUTSIDE_FIB_ZONE_BLOCK_SHORT`、`CONFLICTING_TOO_FAR_NEW_BEAR_LEG_SHORT` 等在 funding positive 环境下整体为负
- 结论：
  - 第一条外部因子候选已经出现，但它更像：
    - `ETH only`
    - `方向性上下文`
    - 优先尝试“压 short / 放 long”的开仓过滤微调
  - 暂不直接回注策略；下一步先围绕 `ETH funding_positive` 做最小实验，再按 `BTC / ETH / 其他币种` 分层复核。

## 2026-04-16 拒绝实验：ETH funding positive 直接放开 MACD_FALLING_KNIFE_LONG

- 假设：
  - 在 `ETH funding_positive` 环境下，单原因 `MACD_FALLING_KNIFE_LONG` 过滤可能过严，尝试只放开这一类 long。
- 实现方式：
  - 代码层读取最近 `4H` funding context。
  - 仅当 `ETH`、`funding_rate >= 0`、`filter_reasons = ["MACD_FALLING_KNIFE_LONG"]` 且无 short 冲突时，强制恢复 long 入场。
- ETH 回测：
  - 基线：`1428`
  - 实验：`1448`
...
- 命中结果：
  - `MACD_FALLING_KNIFE_LONG` 单原因过滤从 `245` 降到 `237`，实际放开 `8` 个过滤点。
  - 路径变化后新增 `12` 笔 long，合计 `-1245.4224`，仅 `2` 笔盈利。
  - 少了一笔原基线盈利 long，`+35.0824` 未再出现。
  - 同时间同方向交易的净变化只有小幅正向，主要损失来自新增 long 的尾部亏损。
- 结论：
  - ETH 先行闸门失败，判定 `已验证但无效`。
  - 失败原因不是研究因子无价值，而是“funding positive + 单原因 MACD filter”仍过粗，放出了多笔历史负样本，且改变交易路径后尾部亏损放大。
  ...
  - 策略回注代码已撤销，正式基线保持：
    ...
    - `BCH 1431`
- 下一步：
  - 继续保留研究系统结论，但下一轮应先在研究层增加二级分桶，例如 `funding_positive + trend_state / macd_histogram / volatility`，不要直接从 funding 单桶放开入场。

## 2026-04-16 研究系统增强：funding 二级上下文与过滤原因分桶

- 背景：
  - 第一轮直接放开 `funding_positive + MACD_FALLING_KNIFE_LONG` 失败，说明 funding 单桶仍过粗。
  - 需要先在研究系统里拆出更细的环境，而不是继续硬写策略规则。
- 本次实现：
  - 新增 funding 二级上下文因子：
  ...
  - `funding_filter_context` 桶结构：
    - `funding_bucket + direction + primary_filter_reason + ema_distance_state + leg_state`
  - 对 filtered-only 的 ETH 强正桶增加候选判定：
    - `sample_count >= 4`
    - `win_rate >= 70%`
    - `avg_pnl > 0.02`
    - `SharpeProxy >= 0.5`
...
- 假设：
  - 只放开研究系统筛出的窄桶：
    - `ETH`
    - `funding_rate >= 0`
    - `filter_reasons = ["MACD_FALLING_KNIFE_LONG"]`
    - `ema_distance_filter.state = TooFar`
    - `leg_detection_value.is_bullish_leg = true`
- ETH 回测：
  - 基线：`1428`
  - 实验：`1449`
...
- 命中结果：
  ...
  - 少了一笔原基线盈利 long，`+35.0824` 未再出现。
  - 虽然比上一轮粗放规则亏损收敛很多，但仍未通过 ETH 先行闸门。
- 结论：
  - 判定 `已验证但无效`。
  - 不进入 `BTC / SOL / BCH` 分层复核。
  - 策略回注代码已撤销，正式基线保持：
    ...
    - `BCH 1431`
- 下一步：
  - 不再继续尝试“放开 MACD_FALLING_KNIFE_LONG”方向。
  - 研究系统应转向寻找“过滤掉低 Sharpe 开仓环境”，优先从 `funding_positive_short` 的负样本和 `funding_negative` 的保护性过滤出发。

## 2026-04-16 拒绝实验：ETH funding negative + MACD weakening short 过滤

- 假设：
  - `ETH funding_negative_short` 是已成交低 Sharpe 坏桶。
  - 进一步拆分后，`funding_rate < 0` 且 `MACD histogram_decreasing` 的 short 可能属于“动能仍在恶化却追空”的低质量开仓环境。
- 目标样本：
  - `2026-03-22 04:00:00` short：`-352.8743`
  - `2026-03-25 20:00:00` short：`-95.2396`
  - 同类中 `2026-01-22 00:00:00` short 的 `histogram_decreasing=false`，不应被命中。
- ETH 回测：
  - 基线：`1428`
  - 实验：`1450`
  ...
  - `max_drawdown`：`32.3410% -> 32.3410%`
- 命中/路径结果：
  - 实验后少了 `3` 笔基线交易，净 `+444.5043`，其中包含一笔 `2026-01-20 12:00:00` 的大盈利 short：`+504.6616`。
  - 新增 `4` 笔交易，净 `-230.0946`。
  - 过滤确实减少了部分亏损 short，但路径副作用更差，且误伤了盈利更大的 short 环境。
- 结论：
  - ETH 先行闸门失败，判定 `已验证但无效`。
  - 不进入 `BTC / SOL / BCH` 分层复核。
  - 策略回注代码已撤销，正式基线保持：
    ...
    - `BCH 1431`
- 下一步：
  - 不再直接按 funding 正负 + MACD 单条件做策略回注。
  - 研究系统需要先支持“命中交易路径影响”的候选评估，或优先研究不改变入场顺序的权重/止损观测，不再只看单笔 shadow pnl。

## 2026-04-16 研究系统增强：路径影响评估报告

- 背景：
  - `1448 / 1449 / 1450` 都出现了同一个问题：研究分桶或目标坏单看起来有价值，但实际回测利润和 Sharpe 变差。
  - 原因是过滤/放行类规则会改变后续持仓路径，不能只看 shadow pnl、目标样本或单个桶的平均收益。
- 本次实现：
  - 研究系统新增路径影响评估模式。
  - CLI 使用环境变量切换：
  ...
  - 输出维度：
    - 基线有但实验缺失的交易数量 / PnL / 胜笔
    - 实验新增交易数量 / PnL / 胜笔
    - 同标的 + 同方向 + 同开仓时间的共同交易 PnL delta
    - `total_path_delta = new_pnl - missing_pnl + common_delta`
    - `path_improved / path_degraded / neutral`
    - Top changed trades
- 验证命令：
  - `cargo test -p rust-quant-services --test vegas_factor_research -- --nocapture`
  - `DB_HOST='mysql://root:example@localhost:33306/test?ssl-mode=DISABLED' VEGAS_RESEARCH_PATH_BASELINE_ID=1428 VEGAS_RESEARCH_PATH_EXPERIMENT_IDS=1448,1449,1450 VEGAS_RESEARCH_PATH_INST_ID=ETH-USDT-SWAP cargo run -p rust-quant-cli --example run_vegas_factor_research`
- ETH 路径影响结果：
...
- 结论：
  ...
  - 正式基线保持：
    ...
    - `BCH 1431`
- 新增迭代规则：
  - 过滤/放行类规则必须在回测后做路径影响评估。
  - 若 `total_path_delta < 0`，即使命中目标坏单或某个分桶 shadow pnl 正向，也不能升级为正式基线。
  - 下一步优先寻找“不明显改变入场顺序但能改善低 Sharpe 环境”的开仓过滤或权重削弱候选。

## 2026-04-16 研究系统修正：候选语义降级与 Podman 命令对齐

- 背景：
  - 路径影响评估证明 `1448 / 1449 / 1450` 都是 `path_degraded`，但研究报告仍把部分桶标为 `可回注`，语义过强。
  - 本机容器运行环境使用 Podman，不再使用 Docker 命令。
- 本次修正：
  - 将研究报告中的 `FactorConclusion::Candidate` 展示文案从 `可回注` 改为 `可实验`。
  - 在研究报告概览区增加说明：
    - `可实验` 仅代表研究候选；
    - 回注 Vegas 前必须通过路径影响评估。
  - 将 Vegas runbook 和启动指南中的当前容器命令切换为 `podman`。
- 验证：
  - `podman ps` 确认 `mysql` 容器运行。
  - `podman exec -i mysql mysql -uroot -pexample test -e "select ..."` 成功读取 `1428 / 1429 / 1430 / 1431 / 1448 / 1449 / 1450`。
  - `DB_HOST='mysql://root:example@localhost:33306/test?ssl-mode=DISABLED' cargo run -p rust-quant-cli --example run_vegas_factor_research` 输出 `可实验` 和路径影响评估说明。
- 结论：
  - 当前研究系统输出不再暗示“可直接回注”。
  - 后续流程固定为：
    - `可实验候选 -> 单假设策略实验 -> 路径影响评估 -> ETH delta -> 分层复核 -> 是否升级基线`。

## 2026-04-16 研究系统增强：低 Sharpe 开仓环境候选摘要

- 背景：
  - 研究报告长表过大，容易继续人工扫桶并重复尝试已拒绝方向。
  - 下一步需要先把“最差的已成交环境”自动排出来，再判断是否进入单假设实验。
- 本次实现：
  - 在研究报告新增 `低 Sharpe 开仓环境候选` 摘要区。
  - 入选规则：
    - 只看 `已成交样本`
    - `sample_count >= 3`
    - `avg_pnl < 0` 或 `SharpeProxy < 0`
  - 排序规则：
    - `ETH` 优先
    - `SharpeProxy` 更差优先
    - `AvgPnL` 更差优先
...
- 真实输出首位：
  ...
  - `SharpeProxy=-0.27`
- 下钻明细：
  - `2026-01-22 00:00:00` short：`+248.4479`
    - `funding_rate=-0.0000115112548794`
    - `histogram_decreasing=false`
    - `volume_ratio=4.4827`
  - `2026-03-22 04:00:00` short：`-352.8743`
    - `funding_rate=-0.0000218352682131`
    - `histogram_decreasing=true`
    - `volume_ratio=2.6118`
  - `2026-03-25 20:00:00` short：`-95.2396`
    - `funding_rate=-0.0000129269154443`
    - `histogram_decreasing=true`
    - `volume_ratio=2.0552`
- 判断：
  - 这个候选与 `1450` 已拒绝实验高度重叠。
  - 虽然局部仍显示 `histogram_decreasing=true` 可分离坏单，但 `1450` 已证明路径影响为 `-526.48`，不能重复回测同一方向。
- 新增迭代规则：
  - 低 Sharpe 候选只用于排序下一轮实验。
  - 若候选与已拒绝实验条件高度重叠，必须标记为“已覆盖拒绝”，不重复进入策略回测。
- 下一步：
  - 跳过 `funding_negative_short + histogram_decreasing` 方向。
  - 继续寻找没有被路径影响拒绝覆盖的候选，优先考虑样本不改变入场顺序的权重削弱或止损观测。

## 2026-04-16 研究系统修正：候选覆盖分离、分层样本门槛与 MACD 分桶口径

- 背景：
  - 上一轮低 Sharpe 摘要首位 ETH 候选已被 `1450` 覆盖拒绝，但仍出现在候选列表顶部。
  - 剔除已覆盖候选后，BTC 首位候选只有 `3` 笔，且主要由单笔尾部亏损驱动，样本量不足。
  - 下钻 BTC 候选时发现一个数据口径问题：
    - `macd_value.above_zero=false`
    - 但 `macd_value.histogram=731.72`
    - 研究系统原先优先信任 `above_zero`，导致样本被误分进 `macd_below_zero` 桶。
- 本次实现：
  - 低 Sharpe 摘要拆分为：
    - `下一轮未覆盖低 Sharpe 开仓环境候选`
    - `已覆盖拒绝候选`
  - 已覆盖规则：
    - `ETH + funding_negative_short`
    - `ETH + funding_negative_short_mixed_trend`
    - `ETH + funding_negative_short_*macd*`
    - 标记为 `covered_by_1450`
  - 分层最小样本门槛：
    - `ETH >= 3`
    - `BTC >= 5`
    - `其他币种 >= 6`
  - 摘要新增 `TotalPnL = AvgPnL * sample_count`。
  - 同层候选排序改为优先看 `TotalPnL` 影响，再看 `SharpeProxy / AvgPnL`。
  - MACD 分桶修正：
    - 优先使用 `histogram` 数值正负判断 `macd_above_zero / macd_below_zero`
    - 仅当 `histogram` 缺失时才回退到 `above_zero` 布尔字段
- 真实研究输出变化：
  - `BTC funding_positive_long_volume_normal` 因样本数只有 `3`，被剔除。
  - 修正前的 `BTC funding_positive_long_macd_below_zero_hist_flat` 首位候选消失，说明它是 `above_zero` 布尔字段污染造成的假候选。
  - 当前首位未覆盖候选：
    - `funding_trend_context / funding_positive_long_mixed_trend / BTC`
    - `5` 笔
    - `win_rate=40.00%`
    - `AvgPnL=-0.66`
    - `TotalPnL=-3.31`
    - `SharpeProxy=-0.32`
- 下钻 BTC 首位候选：
  - `2026-01-08 20:00:00`：`-2.6275`
  - `2026-01-09 16:00:00`：`+2.8034`
  - `2026-01-12 20:00:00`：`-1.8544`
  - `2026-01-22 00:00:00`：`+0.6346`
  - `2026-01-25 16:00:00`：`-2.2613`
  - 净影响只有约 `-3.31`，不值得立即进入策略实验。
- 结论：
  - 本轮不做策略回注。
  - 研究报告排序更稳健，已避免重复选择 `1450` 覆盖方向，也避免小样本和 MACD 布尔污染造成假候选。
  - 下一步应继续寻找 `TotalPnL` 更大、未被拒绝覆盖、且不依赖可疑派生字段的候选。

## 2026-04-16 研究系统修正：最小影响门槛与其他币种按标的拆分

- 背景：
  - 修正 MACD 分桶后，首位未覆盖候选变成：
    - `funding_trend_context / funding_positive_long_mixed_trend / BTC`
    - `5` 笔
    - `TotalPnL=-3.31`
  - 下钻后发现只是低影响噪音，不值得进入策略回测。
  - 同时，`其他币种 funding_positive` 的 `TotalPnL=-27.86` 看似可执行，但按 `SOL / BCH` 拆分后发现：
    - 主要亏损来自 `SOL funding_positive short` 单笔 `-25.6197`
    - `BCH` 只是多笔小额低影响亏损
  - 因此 `其他币种` 不能作为整体可执行结论。
- 本次实现：
  - `FactorBucketReport` 新增 `scope_label`。
  - BTC/ETH 报告范围保持 `BTC / ETH`。
  - `其他币种` 按具体标的拆为 `SOL / BCH`。
  - 下一轮候选增加最小影响门槛：
    - `TotalPnL <= -10` 才进入 `下一轮未覆盖低 Sharpe 开仓环境候选`
    - 未达门槛的候选进入 `低影响观察候选`
- 真实研究输出：
  - `下一轮未覆盖低 Sharpe 开仓环境候选` 当前为空。
  - `低影响观察候选` 包含：
    - `BTC funding_positive_long_mixed_trend`：`TotalPnL=-3.31`
    - `BCH funding_positive`：`TotalPnL=-2.24`
    - `BCH funding_positive_long`：`TotalPnL=-2.05`
    - `BCH funding_positive_long_macd_above_zero_hist_decreasing`：`TotalPnL=-2.02`
  - `ETH funding_negative_short*` 继续标记为 `covered_by_1450`。
- 结论：
  - 当前没有满足门槛、未被拒绝覆盖、样本数足够的低 Sharpe 开仓环境候选。
  - 本轮不做策略回注。
  - 下一步应扩大研究维度，而不是强行从低影响候选里写规则：
    - 观察止损/出场调节候选
    - 引入 price + OI 或 flow 数据覆盖后再筛
    - 或放宽到“权重削弱”而不是硬过滤

## 2026-04-16 出场侧分析：止损坏桶与 ETH-only 混合趋势候选

- 背景：
  - 低 Sharpe 开仓环境候选已无满足 `TotalPnL <= -10`、未覆盖拒绝、样本足够的可实验项。
  - 本轮不继续扩大开仓过滤，改查正式基线 `1428 / 1429 / 1430 / 1431` 的出场/止损贡献。
- close_type 聚合结果：
...
- stop_loss_source 拆分：
  - `Signal_Kline_Stop_Loss` 不能整体视为坏逻辑：
    - `ETH long Engulfing_Volume_Confirmed`：`54` 笔，`TotalPnL=+2027.2386`
    - `ETH long Engulfing_Volume_Rejected`：`37` 笔，`TotalPnL=+310.7198`
  - 真正拖累集中在：
    - `ETH short KlineHammer_Volume_Confirmed`：`14` 笔，`TotalPnL=-417.6606`
    - `ETH short Engulfing_Volume_Confirmed`：`56` 笔，`TotalPnL=-370.8297`
    - `ETH short Engulfing_Volume_Rejected`：`37` 笔，`TotalPnL=-361.0590`
    - `ETH long KlineHammer_Volume_Confirmed`：`14` 笔，`TotalPnL=-358.0516`
    - `ETH long LargeEntity_Retracement`：`12` 笔，`TotalPnL=-220.9504`
- 降维后的内部环境候选：
...
- 关键样本：
  - `2026-03-25 20:00:00` ETH short，`2026-03-26 00:00:00` 由 `Engulfing_Volume_Confirmed` 触发 `Signal_Kline_Stop_Loss`，`PnL=-95.2396`
  - 同桶最大亏损为 `2025-09-24 12:00:00` ETH long，`KlineHammer_Volume_Confirmed` 止损，`PnL=-149.6078`
- 结论：
  - 该候选具备 ETH 研究价值，但当前只能标记为 `仅 ETH 有效候选`。
  - 不能写成跨币种通用规则，因为 BTC 影响接近噪音，BCH/SOL 不坏。
  - 下一步若继续，应把该条件纳入研究系统的“内部环境/出场侧候选”报告，再做 ETH 单假设策略实验与路径影响评估；不能直接粗暴关闭 `Signal_Kline_Stop_Loss` 或全部吞没/锤子止损。

## 2026-04-16 研究系统增强：出场/止损环境候选摘要

- 背景：
  - 手工 SQL 已确认低 Sharpe 开仓候选为空，但出场/止损侧存在更大的 ETH 拖累。
  - 如果继续靠人工 SQL 查止损坏桶，容易再次把单笔样本写成策略规则。
  - 因此先把内部出场环境纳入研究系统，让报告自动排序，不直接改 Vegas 交易逻辑。
- 本次实现：
  ...
  - 报告新增独立区块：
    - `出场/止损环境候选`
  - `下一轮未覆盖低 Sharpe 开仓环境候选` 明确排除 `exit_environment_context`，避免把出场候选误当开仓过滤候选。
- TDD 验证：
  - 先新增失败测试：
    - `classifies_internal_exit_environment_context_buckets`
    - `renders_report_with_required_sections`
  - 再实现最小代码通过。
- 验证命令：
  - `cargo fmt --all`
  - `cargo test -p rust-quant-services --test vegas_factor_research -- --nocapture`
...
- 真实报告结果：
  ...
  - `出场/止损环境候选` 首位：
    ...
    - `SharpeProxy=-0.78`
  - 其他 ETH 高影响候选：
    - `max_loss_stop_with_trend_macd_align_distance_too_far_volume_expansion`：`5` 笔，`TotalPnL=-361.29`
    - `signal_stop_mixed_trend_macd_align_distance_too_far_volume_expansion`：`16` 笔，`TotalPnL=-297.45`
    - `signal_stop_mixed_trend_macd_against_distance_too_far_volume_expansion`：`6` 笔，`TotalPnL=-281.92`
- 结论：
  - 研究系统现在可以自动输出出场/止损侧候选。
  - 当前最值得继续的不是吞没/锤子单点规则，而是 `max_loss_stop` 在 ETH 的特定内部环境。
  - 下一步应先下钻首位候选的交易明细，判断它是：
    - 入场质量问题
    - 最大止损过宽问题
    - 还是需要动态提前止损/减仓的问题
  - 未完成明细下钻和路径影响评估前，不升级基线。

## 2026-04-16 BTC 动态最大止损分层验证：3%-5% 入场振幅温和降损有效

- 背景：
  - 前一轮固定止损扫描显示：
    - BTC 直接把 `max_loss_percent` 从 `0.05` 降到 `0.045 / 0.04 / 0.035` 均未优于基线。
    - SOL 放大止损会降低 profit / Sharpe，不应继续全局放大。
    - BCH 放大止损仍为负，问题不是止损太窄。
  - 因此本轮不再粗暴改固定止损，而是只验证一个假设：
    - BTC 的坏簇可能集中在 `入场 K 线振幅 3%-5%` 的中等高波动段；
    - 该段需要温和收紧动态最大止损，而不是全局降低固定止损。
- 本次代码改动：
  - `BasicRiskStrategyConfig` 新增动态最大止损分层字段：
    - `dynamic_entry_amp_threshold`
    - `dynamic_entry_loss_percent`
    - `dynamic_entry_require_direction_mismatch`
    - `dynamic_range_threshold`
    - `dynamic_range_loss_percent`
  - `compute_effective_max_loss` 保持默认兼容：
    - 默认 `entry_amp > 0.03`
    - 默认要求方向不利才触发
    - 默认触发后收紧到 `0.03`
    - 默认运行中振幅 `> 0.05` 收紧到 `0.045`
  - 新增配置化入口 `compute_effective_max_loss_with_config`。
  - `compute_current_targets / check_max_loss_stop / check_risk_config_with_r_system` 统一使用配置化动态止损。
  - 修复 DB 配置链路：
    - `strategy_config.risk_config -> BasicRiskStrategyConfig -> ParamMergeBuilder -> 回测执行`
    - 避免新增字段在 `ParamMergeBuilder::to_risk_config` 中被丢弃。
- TDD / 验证：
  - 新增测试：
    - `effective_max_loss_keeps_default_entry_mismatch_tightening`
    - `effective_max_loss_can_tighten_entry_amplitude_without_direction_mismatch`
    - `to_risk_config_preserves_dynamic_max_loss_thresholds`
  - 验证命令：
    - `cargo fmt --all`
...
- BTC 实验结果：
  - 基线 `1483`：
    ...
    - `profit=372.253`
    - `sharpe=1.13633`
    - `max_drawdown=0.383762`
  - `1501`：`entry_amp > 0.03`，不要求方向不利，收紧到 `0.03`
    - `win_rate=0.553542`
    - `profit=290.235`
    - `sharpe=0.992148`
    - `max_drawdown=0.373088`
    - 结论：止损过紧，拒绝。
  - `1502`：`entry_amp > 0.03`，不要求方向不利，收紧到 `0.035`
    - `win_rate=0.561258`
    - `profit=399.423`
    - `sharpe=1.19698`
    - `max_drawdown=0.355666`
    - 结论：接受，BTC 当前最佳分层动态止损候选。
  - `1503`：`entry_amp > 0.05`，不要求方向不利，收紧到 `0.035`
    - `win_rate=0.560531`
    - `profit=372.190`
    - `sharpe=1.13960`
    - `max_drawdown=0.362347`
    - 结论：改善主要来自 `3%-5%` 入场振幅段，而不是 `5%+` 极端段。
- BCH 复核结果：
  - 基线 `1485`：
    - `win_rate=0.393496`
    - `profit=-56.4274`
    - `sharpe=-0.338972`
    - `max_drawdown=0.655402`
  - `1504`：`entry_amp > 0.08`，不要求方向不利，收紧到 `0.03`
    - `win_rate=0.396411`
    - `profit=-50.367`
    - `sharpe=-0.294636`
    - `max_drawdown=0.662556`
    - 结论：略减亏但回撤变差，拒绝。
  - `1505`：`entry_amp > 0.05`，不要求方向不利，收紧到 `0.03`
    - `win_rate=0.389338`
    - `profit=-55.1833`
    - `sharpe=-0.332208`
    - `max_drawdown=0.631929`
    - 结论：回撤改善但收益/胜率仍弱，拒绝。
- 最终 DB 配置：
  ...
  - BCH `strategy_config.id=13` 已移除实验字段，恢复原风险配置。
  - SOL 本轮不动，因为分桶显示高波动桶不是主要坏簇。
- 底层原因判断：
  - BTC 属于低波动、高趋势连续性的层级，`3%-5%` 入场振幅已经代表风险状态抬升。
  - 直接把固定止损降到 `3%-4.5%` 会误杀正常趋势回撤；但只在入场振幅超过 `3%` 时温和收紧到 `3.5%`，能减少中等异常波动下的尾部亏损，同时保留趋势单的呼吸空间。
  - `1501` 证明 `3%` 太紧会砍掉收益；`1503` 证明只处理 `5%+` 太晚，无法捕获主要改善段；`1502` 位于两者之间，因此 profit / Sharpe / maxDD 同时改善。
  - BCH 未通过说明该因子不是单参数通用；BCH 的主要问题更接近入场分布本身无正期望，不能靠单一止损参数修复。
- 分类结论：
  - `BTC`：接受，分层参数有效。
  - `BCH`：拒绝，恢复原配置。
  - `SOL`：未参与本轮，保持原配置。
