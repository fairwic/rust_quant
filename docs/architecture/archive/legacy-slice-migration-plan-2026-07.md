# 归档：按切片与 Registry 治理的 Rust Quant 迁移计划

> 本文是 2026-07 的历史迁移计划，保留当时的业务发现和 Contract 细节。其 Manifest、Registry、源码哈希和切片执行流程已被 ADR-0017 取代，不得作为新的迁移入口。归档后的相对链接按原目录理解。
