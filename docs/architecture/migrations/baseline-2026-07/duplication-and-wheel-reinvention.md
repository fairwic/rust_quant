...
| D4 | **K线模型/聚合/对齐重复**,8+ 个 OHLCV 结构体 | `common CandleItem`(权威)、`domain Candle`、`market CandlesEntity`、MVE `BacktestCandle`、tv-parity `Candle`、`KlineScanCandle` 等;对齐两份(`confirmed_candle_aggregator.rs:463` vs `market_velocity_backfill.rs:538`) | 8+ 结构体 | `common::CandleItem` 唯一内存模型,其余仅 DB 实体 + `From`;对齐/聚合统一到 `market::candle` 一份 |
