...
fn explain_strength_calculation(level: &PriceLevel, is_near_current_price: bool) -> String {
    ...
    explanation.push(format!(
        "确认次数({})贡献: +{:.2}",
        level.confirmation_count, confirmation_factor
    ));
    ...
    } else if level.price.to_string().contains(".5")
        || level.price.to_string().contains(".236")
    ...
    // 聚类奖励估算 (多个相近价格合并)
    let cluster_bonus = if level.strength > 0.8 && level.confirmation_count > 10 {
        0.15
    } else {
        0.0
    };
    ...
    // 总计估算强度
    let estimated_total = (base_strength
        + confirmation_factor
        + fib_factor
        + round_factor
        + proximity_factor
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // 使用Binance API获取数据
    let candles =
        trading::task::basic::get_candle_data_confirm("BTC-USDT-SWAP", "1H", 1000, None).await?;
    ...
    let strongest_support = indicator
        .get_strongest_support(&support_levels)
        .expect("没有找到支撑位");

    let strongest_resistance = indicator
        .get_strongest_resistance(&resistance_levels)
        .expect("没有找到压力位");
    ...
    println!(
        "确认次数: {} (价格在这一水平获得支撑的次数)",
        strongest_support.confirmation_count
    );
    ...
    println!(
        "确认次数: {} (价格在这一水平受阻回落的次数)",
        strongest_resistance.confirmation_count
    );
    println!("类型: {}", strongest_resistance.type_name);

    // 添加强度计算详细内容
    println!("\n强度计算详情:");
    let resistance_is_near =
        ((strongest_resistance.price - current_price) / current_price * 100.0) < 3.0;
    let resistance_explanation =
        explain_strength_calculation(strongest_resistance, resistance_is_near);
