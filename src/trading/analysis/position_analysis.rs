...
impl PositionAnalysis {
    pub async fn analyze_positions(back_test_id: i32, candles: &[CandlesEntity]) -> Result<()> {
        info!("Starting position analysis for back_test_id: {}", back_test_id);
        
        // 创建分析模型
        let analysis_model = BackTestAnalysisModel::new().await;
        
        // 查询需要分析的持仓记录
        let positions = analysis_model.find_positions(back_test_id).await
            ...
            tasks.push(task::spawn(async move {
                ...
                for position in positions_chunk {
                    match position.open_price.parse::<f64>() {
                        Ok(open_price) => {
                            // 查找开仓时间对应的K线索引
                            if let Some(open_index) = find_candle_index(&candles, &position.open_position_time) {
                                // 分析不同K线数量后的价格变化
                                for bars in &bars_to_analyze {
                                    ...
                                    if let Ok(future_price) = candles[open_index + *bars as usize].c.parse::<f64>() {
                        ...
                        Err(e) => {
                            error!("Failed to parse open price '{}': {}", position.open_price, e);
                        }
                    }
                }
        ...
        if !all_analyses.is_empty() {
            let affected_rows = analysis_model.batch_insert(all_analyses).await
                .context("Failed to insert analysis records")?;
            info!("Successfully inserted {} analysis records for back_test_id: {}", affected_rows, back_test_id);
            
            // 计算统计数据并更新 back_test_log
            info!("开始计算 back_test_id {} 的K线后胜率统计", back_test_id);
            let stats = analysis_model.calculate_position_stats(back_test_id).await
            ...
            let updated = log_model.update_position_stats(back_test_id as i64, stats).await
                .context("Failed to update back_test_log with position statistics")?;
            
            info!("成功更新 back_test_id: {} 的统计数据到 back_test_log 表 (影响行数: {})", back_test_id, updated);
        } else {
            info!("No analysis records to insert for back_test_id: {}", back_test_id);
        }
