...
impl KeyLevelIndicator {
    ...
    fn calculate_point_strength(&self, data: &[DataItem], index: usize, is_high: bool) -> f64 {
        ...
        // 考虑成交量因素
        let avg_volume = data[index - 5..index]
            .iter()
            .map(|x| x.volume())
            .sum::<f64>()
            / 5.0;
        ...
        // 考虑时间因素(越近的点越重要)
        let recency_factor =
            1.0 + (index as f64 / data.len() as f64) * (self.recent_levels_boost - 1.0);

        // 计算确认次数因素
        let price_to_check = if is_high {
            data[index].high()
        } else {
            data[index].low()
        };
    ...
    fn identify_ma_levels(&self, data: &[DataItem]) -> Vec<PriceLevel> {
        ...
        for &period in &self.ma_periods {
            ...
            // 计算最后一个MA值
            let ma_value = data[(data.len() - period)..]
                .iter()
                .map(|x| x.close())
                .sum::<f64>()
                / period as f64;
    ...
    fn count_ma_confirmations(&self, data: &[DataItem], period: usize, is_support: bool) -> u32 {
        ...
        for i in period..data.len() {
            // 计算当前点的移动平均
            let ma = data[(i - period)..i].iter().map(|x| x.close()).sum::<f64>() / period as f64;
    ...
    fn identify_psychological_levels(&self, data: &[DataItem]) -> Vec<PriceLevel> {
        ...
        while price <= max_price * 1.05 {
            // 稍微超出最高价，为未来波动预留空间
            // 整数关卡
            if self.is_psychological_level(price) {
                let is_support = price < current_price;
                let distance_factor =
                    1.0 - ((price - current_price).abs() / (max_price - min_price)).min(1.0);
    ...
    fn cluster_price_levels(&self, levels: Vec<PriceLevel>) -> Vec<PriceLevel> {
        ...
        for cluster in clusters {
            ...
            let weighted_price: f64 =
                cluster.iter().map(|l| l.price * l.strength).sum::<f64>() / total_weight;

            // 确定支撑/压力类型(按多数决定)
            let support_count = cluster.iter().filter(|l| l.type_name == "支撑").count();
    ...
    // 增加一个增强处理最近历史点位的方法
    fn enhance_recent_levels(
        &self,
        mut levels: Vec<PriceLevel>,
        data: &[DataItem],
    ) -> Vec<PriceLevel> {
        // 获取全部数据的长度作为参考
    ...
    pub fn get_strongest_support<'a>(
        &self,
        support_levels: &'a [PriceLevel],
    ) -> Option<&'a PriceLevel> {
    ...
    pub fn get_strongest_resistance<'a>(
        &self,
        resistance_levels: &'a [PriceLevel],
    ) -> Option<&'a PriceLevel> {
    ...
    // 这是可选功能，如果有需要的话
    pub fn get_closest_significant_support<'a>(
        &self,
        support_levels: &'a [PriceLevel],
        current_price: f64,
        min_strength: f64,
    ...
    pub fn get_closest_significant_resistance<'a>(
        &self,
        resistance_levels: &'a [PriceLevel],
        current_price: f64,
        min_strength: f64,
