...
crud!(TakerVolumeEntity {}, "taker_volume");

// 如果需要自定义的查询/更新语句，也可用 impl_select/impl_update 宏
// 这里演示一个fetch_list()，可获取表内所有行
impl_select!(TakerVolumeEntity{fetch_list() => ""},"taker_volume");

// 可自定义带条件的查询，示例:
impl_select!(TakerVolumeEntity{select_by_inst_id(inst_id: &str,period:&str ) => "`where inst_id = #{inst_id} and period= #{period}  limit 1 `"},"taker_volume");

// 若需要自定义更新，也可这样写:
// impl_update!(TakerVolumeEntity{update_by_id(id:i32) => "`where id = #{id}`"},"taker_volume");

// 可自定义带条件的查询，示例:
impl_select!(TakerVolumeEntity{select_new_one_data(inst_id: &str,period:&str ) ->Option=> " `where inst_id = #{inst_id} and period= #{period}  order by ts desc  limit 1 "},"taker_volume");

impl_select!(TakerVolumeEntity{select_older_one_data(inst_id: &str,period:&str ) ->Option=> " `where inst_id = #{inst_id} and period= #{period}  order by ts asc  limit 1 "},"taker_volume");
...
pub struct TakerVolumeModel {
    db: &'static RBatis,
}

impl TakerVolumeModel {}
...
impl TakerVolumeModel {
    ...
    pub async fn get_oldest_data(
        ...
        inst_id: &str,
        time_interval: &str,
    ) -> Result<Option<TakerVolumeEntity>> {
        let res = TakerVolumeEntity::select_older_one_data(self.db, inst_id, time_interval).await;
        match res {
            Ok(list) => Ok(list),
    ...
    /// 批量插入
    pub async fn add_list(&self, list: Vec<TakerVolumeEntity>) -> Result<()> {
        // insert_batch是CRUD宏生成的方法之一
        let data = TakerVolumeEntity::insert_batch(self.db, &list, list.len() as u64).await?;
    ...
    pub async fn get_new_one_data(
        ...
        inst_id: &str,
        period: &str,
    ) -> anyhow::Result<Option<TakerVolumeEntity>> {
        let res = TakerVolumeEntity::select_new_one_data(self.db, inst_id, period).await;
        match res {
            Ok(list) => Ok(list),
            Err(_) => Err(anyhow!("db error ")),
        }
    }
    pub(crate) async fn get_new_count(&self, inst_id: &str, period: &str) -> Result<u64> {
        let mut query = format!(
            "select count(1) from {} where inst_id = '{}' AND period = '{}' ;",
            "taker_volume", inst_id, period
        );
        debug!("query: {}", query);
