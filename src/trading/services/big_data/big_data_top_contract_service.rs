...
impl TopContractServiceTrait for BigDataTopContractService {
    ...
    async fn init(&self,
        ...
        println!("init_long-short-account-ratio-contract-top-trader...");
        let limit = 1440; // 设置限制
        for inst_id in inst_ids {
            for period in periods.iter() {
                //延迟100ms
...
impl BigDataTopContractService {
    ...
    async fn get_initial_begin_with_end(
        ...
        let res = TopContractAccountRatioModel::new()
            .await
    ...
    fn process_okx_data(
        ...
        Ok(res
            ...
            .filter_map(|row| {
                ...
                let long_short_acct_ratio = row.get(1)?.to_string();
                Some(ModelEntity {
                    id: 0,
                    ts,
                    inst_id: inst_id.to_string(),
                    period: period.to_string(),
                    long_short_acct_ratio,
                    created_at: None,
                })
            })
            .collect())
    ...
    async fn is_limit_reached(inst_id: &str, period: &str, limit: i64) -> anyhow::Result<bool> {
        let count = TopContractAccountRatioModel::new()
            .await
    ...
    pub async fn get_list_by_time(
        ...
        let mut data = TopContractAccountRatioModel::new().await.get_all(inst_id, period, limit,offset, select_time).await?;
        if !data.is_empty() {
            //数据进行反向排序
