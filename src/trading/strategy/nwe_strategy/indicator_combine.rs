use crate::{trading::{indicator::{atr::ATR, nwe_indicator::NweIndicator, rsi_rma_indicator::RsiIndicator, volume_indicator::VolumeRatioIndicator}, strategy::nwe_strategy::{NweSignalValues, NweStrategyConfig}}, CandleItem};
...
impl NweIndicatorCombine {
    pub fn new(config: NweStrategyConfig) -> Self {
        Self {
            ...
            atr_indicator: Some(ATR::new(config.atr_period).expect("ATR period must be > 0")),
        }
    }
    ...
    pub fn get_indicator_values(&mut self, nwe_signal_values: &mut NweSignalValues, data_item: &CandleItem) -> NweSignalValues {
        ...
        nwe_signal_values.atr_value = self.atr_indicator.as_mut().unwrap().next(data_item.h, data_item.l, data_item.c);
