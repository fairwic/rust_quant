...
async fn test_run_strategy_job() -> Result<()> {
    ...
    //volume
    strategy.volume_signal.as_mut().unwrap().volume_increase_ratio = 2.0;
    ...
    let time = "1Dutc";
    //初始化数据与获取初始化的指标值
    let result = StrategyOrder::initialize_strategy_data(&strategy_config, &*"BTC-USDT-SWAP".to_string(),
                                                         &*"1Dutc".to_string()).await;
